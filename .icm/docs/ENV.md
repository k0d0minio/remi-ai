...
- **Where set** — `Vercel` (app runtime), `Actions` (CI secret), `both`, `local` (a developer's
  own shell), or `repo` (checked in, so every clone and every agent session already has it).
