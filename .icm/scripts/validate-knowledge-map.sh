...
# Verdict (stdout, last line): RESULT: OK 0 · RESULT: SKIP 0 (no map, or no docs_path) ·
#   RESULT: INVALID exit 2 (paths that do not resolve are listed above)
set -euo pipefail
...
[ -d "$docs_path" ] || die "docs_path '$docs_path' (from .icm/project.json) is not a directory"
...
while IFS= read -r p; do
  ...
  p="${p#"$docs_path"/}"
  case "$p" in
    .icm/*|.claude/*|.github/*|*.sh|*/\*|*\**|*\<*|http*) continue ;;
  esac
  checked=$((checked + 1))
