...
**capability** skills live flat in `.claude/skills/` and are _called by_ stages (none yet —
`.claude/SKILLS.md`).
...
and hands the changelog page's one-liner to `scripts/notify.sh` (`_shared/project-rules.md` →
Announcing).
...
owns its shape); its H1 is the one-liner `scripts/notify.sh` sends after the merge.
...
| The changelog page's shape                                        | `.icm/_shared/project-rules.md` → Announcing                               |
