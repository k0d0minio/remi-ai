...
| **Priority**   | P0 — publicly visible fabricated contract terms; not gated on the client call                                                   |
| **Size**       | An hour                                                                                                                         |
| **Depends on** | —                                                                                                                               |
| **Blocked by** | —                                                                                                                               |
...
mistook for a contract, and the entire V2 plan was rebuilt on that correction.
...
consciously deferred this artefact; REMI-008's acceptance is scoped to `apps/docs` only.
No other open ticket covers it. It is the exact artefact that misled one audit already,
and it misrepresents the client relationship to anyone who finds the URL.
...
2. Sweep `apps/admin` for any other copy asserting signed pilot terms, the €24.50 figure,
   or the 1 September date, and remove it. Do not touch `apps/docs` business pages — that
   is REMI-008's scope.
