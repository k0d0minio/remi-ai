...
- [ ] Deleting a patient removes their summary row: the `patient_id` foreign key is
      `ON DELETE cascade` in the schema and the generated migration (the in-memory test client
      enforces no foreign key, so cascade is verified in the migration SQL, as it is for every
      other patient table).
...
- [ ] New service tests cover round-tripping through the in-memory client: unwritten returns `null`,
      insert-then-update keeps one row, clear-to-empty removes the row, re-saving the same words is a
      no-op, and `touchPatient` fires on a real write.
