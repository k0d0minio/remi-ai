...
| root `package.json` → `pnpm.overrides.zod`  | zod 4.4.0 made `z.custom()` reject `undefined`. Nextra validates its `<Layout>` props with `z.custom()` **after** destructuring `children` out of them, so `children` is always undefined and every page fails to prerender with `expected nonoptional`. Nextra asks for `^4.1.12`, which resolves to 4.4.x. Pinned to 4.3.5, the last version that passes. | Nextra validates the props it actually received |
