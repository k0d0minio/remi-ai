...
/**
 * Placard & frigo: the essentials in her order, each with its why (§ H).
 *
...
const Segment = async ({ params }: { params: Promise<Params> }) => {
  ...
  if (!data || !hasSegment(data, "placard-frigo")) {
    notFound();
  }

  return (
    <SegmentPage title={content.pantryTitle}>
      <PantryList essentials={data.essentials} content={content} />
    </SegmentPage>
  );
};
